...
class Draggable extends Component {
  ...
  drop(dropedSort, data, sortKey, dropedUid, codeKey, ee) {
    ...
    if (uId === dropedUid) {
      ...
      if (
        data.filter(item => item[codeKey] === objDragedItem[codeKey]).length ===
        0
      ) {
        const maxSort = Math.max.apply(
          Math,
          data.map(citem => citem[sortKey])
        );
        data.map(item => {
    ...
    // const newData = this.props.onChange(data);
    // return newData;
  }
  allowDrop(ee) {
    ee.preventDefault();
  ...
  // 生成拖拽组件
  createDraggleComponent(data, sortKey, style, uId, codeKey) {
    // console.log(data.sort(this.compare(sortKey)));
    return data.sort(this.compare(sortKey)).map(item => {
      return (
        <div
          ...
          onDragStart={this.domdrugstart.bind(
            this,
            item[sortKey],
            item[codeKey],
            uId,
  ...
  render() {
    let newValue;
    const { value, sortKey, codeKey, style } = this.props;
    ...
    return (
      <div>
        ...
        >
          {this.createDraggleComponent(newValue, sortKey, style, uId, codeKey)}
        </div>
      </div>
    );
  }
