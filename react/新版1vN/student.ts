  ...
  .views((self) => {
    return {
      ...
      get shouldCheckDevice(): boolean {
        return (
          self.invited && self.deviceCheckStatus === MIC_CHECK_STATUS.NOT_PASS
        );
      },
      get shouldShowNotation(): boolean {
        return (
          self.invited &&
          self.deviceCheckStatus === MIC_CHECK_STATUS.NOT_AVAILABLE
        );
      },
      get shouldShowConfirm(): boolean {
        return (
          self.invited && self.deviceCheckStatus === MIC_CHECK_STATUS.AVAILABLE
        );
      },
